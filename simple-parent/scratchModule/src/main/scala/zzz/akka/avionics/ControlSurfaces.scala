...
class ControlSurfaces(altimeter: ActorRef) extends Actor {
  ...
  def receive = {
    ...
    // This is a tell call ( ! ) as opposed to a
    // ask call ( ? ) which returns a future
    // The ControlSurface here recieves a stick back 
    // message and then sends it to the Altimeter 
