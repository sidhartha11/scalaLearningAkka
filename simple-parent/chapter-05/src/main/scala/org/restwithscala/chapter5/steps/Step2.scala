...
object AkkaHttpDSLStep2 extends App {
  ...
  implicit val StringMarshaller: ToEntityMarshaller[Task] =
    Marshaller.opaque { 
//    s ⇒ HttpEntity(ContentType(`text/plain`), s.toString) 
    s ⇒ HttpEntity(`text/plain` withCharset `UTF-8`, s.toString) 
  ...
  println("sampleHeader=%s".format(sampleHeader))
  ...
  // handle the /tasks part of the request
    path("tasks") {
      ...
      path("tasks" / LongNumber) {
        println("entered tasks/LongNumber")
        task => {
          println("1 entered task block:task=%s".format(task))
          entity(as[String]) { body => {
            println("entered entity(as[String]) body = %s".format(body))
            put {
              println("entered put block")
              complete {
          ...
          get {
            ...
            ( setCookie(HttpCookie(name = "hello", value = "world")) &
              ...
              println("calling onComplete for %s".format(task))
