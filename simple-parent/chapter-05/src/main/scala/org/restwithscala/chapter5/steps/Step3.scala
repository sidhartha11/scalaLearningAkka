...
object AkkaHttpDSLStep3 extends App {
 ...
 //     HttpEntity(ContentType(`text/plain`), s.toString) }
      HttpEntity(`text/plain` withCharset `UTF-8`, s.toString) }
...
//   implicit def customExceptionHandler = ExceptionHandler {
  val customExceptionHandler = ExceptionHandler {
    case _: IllegalArgumentException =>
      println("in case _ of customExceptionHandler")
      // you can easily access any request parameter here using extractors.
      extractUri { uri =>
        println(" uri of extractUri = %s".format(uri))
  ...
  // handle the /tasks part of the request
    path("search") {
      println("in search part")
      handleExceptions(customExceptionHandler) {
        println("in handleExceptions block")
